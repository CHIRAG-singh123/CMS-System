...
export function SidebarBody({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return (
    <div
      ...
      className={clsx(
        className,
        'scrollbar-hidden flex flex-1 flex-col overflow-y-auto p-4 [&>[data-slot=section]+[data-slot=section]]:mt-8'
      )}
    />
  )
}
...
export const SidebarItem = forwardRef(function SidebarItem(
  ...
  return (
    <span className={clsx(className, 'relative')}>
      {current && (
        <span className="absolute inset-y-2 -left-4 w-0.5 rounded-full bg-zinc-950 transition-opacity dark:bg-white" />
      )}
